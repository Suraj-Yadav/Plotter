...
public:
	...
	Func(const std::string &n, int A) {
		name = n;
		noOfArgs = A;
	}
	bool operator<(const Func &op) const{
	...
	void setDefinition(const std::vector<term> &postFix) {
		definition.createTree(postFix);
	}
    term *retExp() const{
    	return definition.retTerm();
    }
};
std::set<Func> UserDef;
...
double getPreDef(int which, const std::vector<double> &args) {
	switch (which) {
		case 1:		return sin(args[0]);
		case 2:		return cos(args[0]);
		case 3:		return tan(args[0]);
		case 4:		return asin(args[0]);
		case 5:		return acos(args[0]);
		case 6:		return atan(args[0]);
		case 7:		return atan2(args[0], args[1]);
		case 8:		return sinh(args[0]);
		case 9:		return cosh(args[0]);
		case 10:	return tanh(args[0]);
		case 11:	return asinh(args[0]);
		case 12:	return acosh(args[0]);
		case 13:	return atanh(args[0]);
		case 14:	return ceil(args[0]);
		case 15:	return floor(args[0]);
		case 16:	return fabs(args[0]);
		case 17:	return log10(args[0]);
		case 18:	return log(args[0]) / log(args[1]);
		case 19:	return log(args[0]);
		case 20:	return sqrt(args[0]);
		case 21:	return 3.14159265358979323846;
		case 22:	return 2.7182818284590452354;
		default:	return 0;
	}
}
bool isFunc(const std::vector<term> &infix){
//    if(infix.at(0).type==TermType::CONST&&infix.at(0).type==TermType::)
	return false;
}
