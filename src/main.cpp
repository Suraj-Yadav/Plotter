...
int main() {
	...
	if (!font.loadFromFile("other\\arial.ttf"))
		return -1;
	...
	entries.emplace_back(200, 25);
	entries[0].setColor(sf::Color::Red);
	entries[0].setPosition(20, 20);
	entries[0].setFont(font);
	curves.emplace_back();
	curves[1].setExpression("tan(x)");
	curves[1].setColor(sf::Color::Blue);
	entries.emplace_back(200, 25);
	entries[1].setColor(sf::Color::Red);
	entries[1].setPosition(20, 50);
	entries[1].setFont(font);
	...
	while (window.isOpen()) {
		...
		while (window.pollEvent(event)) {
			//		do{
			// Close window: exit
			if (entries[0].handleEvent(event, sf::Mouse::getPosition(window)))
				;
			if (entries[1].handleEvent(event, sf::Mouse::getPosition(window)))
				;
				...
				graphView = window.getView();
				graphView.setSize(newSize.x * graphView.getSize().x / origSize.x,
								  newSize.y * graphView.getSize().y / origSize.y);
				window.setView(graphView);
				origSize = newSize;
